...
function sanitize(body = {}) {
  ...
  // Map snapshot + track (MapRecorder)
  if (typeof out.mapSnapshotDataUrl === 'string') out.mapSnapshotDataUrl = out.mapSnapshotDataUrl.trim();

  if (Array.isArray(out.track)) {
    out.track = out.track
      .map((p) => {
        ...
        const accuracy = p?.accuracy === null || p?.accuracy === undefined ? null : Number(p.accuracy);
        const speed = p?.speed === null || p?.speed === undefined ? null : Number(p.speed);
        return {
          lat,
          lng,
