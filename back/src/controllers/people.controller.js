...
export async function list(req, res, next) {
  try {
    ...
    // Filtros (compatibles):
    // - branchId / positionId (1 valor)
    // - branchIds / positionIds (CSV o array) => $in
    const branchIdsRaw = req.query.branchIds ?? req.query.branchId ?? '';
    const positionIdsRaw = req.query.positionIds ?? req.query.positionId ?? '';
    ...
    const branchIds = parseIds(branchIdsRaw);
    const positionIds = parseIds(positionIdsRaw);

    if (branchIds.length === 1) filter.branchId = branchIds[0];
    else if (branchIds.length > 1) filter.branchId = { $in: branchIds };

    if (positionIds.length === 1) filter.positionId = positionIds[0];
    else if (positionIds.length > 1) filter.positionId = { $in: positionIds };
...
export async function remove(req, res, next) {
  try {
    ...
    // Requerimiento RRHH (módulo Gold):
    // Eliminación física para evitar que el registro siga existiendo en DB
    // cuando el usuario elige "Eliminar" desde el Form.
