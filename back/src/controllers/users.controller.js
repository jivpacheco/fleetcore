  ...
  create: async (req, res, next) => {
    try {
      ...
      const user = new User({
        ...
        local: {
          allowLocalLogin:
            local.allowLocalLogin !== undefined
              ? !!local.allowLocalLogin
              : true,
        ...
        providers: {
          microsoft: {
            allowMicrosoftLogin:
              providers?.microsoft?.allowMicrosoftLogin !== undefined
                ? !!providers.microsoft.allowMicrosoftLogin
                : true,
          },
        },
    ...
    try {
      ...
      if (Array.isArray(roles)) {
        ...
        if (!nextRoles.length) {
          return res
            .status(400)
            .json({ message: "Debe seleccionar al menos un rol." });
        }
      ...
      const willHaveAdmin = Array.isArray(roles)
      ...
      if (hadAdmin && !willHaveAdmin) {
        ...
        if (otherAdmins === 0) {
          return res.status(409).json({
            message:
              "No se puede quitar el rol admin: este es el último administrador del sistema.",
          });
        }
      }
      ...
      if (providers?.microsoft?.allowMicrosoftLogin !== undefined) {
        ...
        user.providers.microsoft.allowMicrosoftLogin =
          !!providers.microsoft.allowMicrosoftLogin;
      }
