...
// -----------------------------------------------------------------------------
// VMRS integration:
// - vmrs es mapeo opcional (NO diagnóstico).
// - filtros opcionales por vmrsSystemCode/vmrsComponentCode/vmrsSymptomCode.
// -----------------------------------------------------------------------------
...
function cleanVmrsCode(v) {
    ...
    if (!s) return ''
    // VMRS suele ser dígitos y guiones; toleramos A-Z por si hay variantes internas.
...
function sanitize(body = {}) {
    ...
    if (out.vmrs && typeof out.vmrs === 'object') {
        ...
        // Normalización: no guardar niveles “huérfanos”
...
export async function list(req, res, next) {
    try {
        ...
        if (req.query.vmrsSymptomCode) {
            const c = cleanVmrsCode(req.query.vmrsSymptomCode)
            if (c) filter['vmrs.symptomCode'] = c
        }
