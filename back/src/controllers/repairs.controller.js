...
// -----------------------------------------------------------------------------
// VMRS integration:
// - vmrs es mapeo técnico opcional recomendado (para sugerencias/analytics).
// - filtros opcionales por vmrsSystemCode/vmrsComponentCode/vmrsJobCode.
// -----------------------------------------------------------------------------
...
function sanitize(body = {}) {
    ...
    if (typeof out.operationalImpactDefault === 'string')
        out.operationalImpactDefault = U(out.operationalImpactDefault.trim())
    if (out.operationalImpactDefault && !['NO_STOP', 'LIMITED', 'OUT_OF_SERVICE'].includes(out.operationalImpactDefault)) {
        const err = new Error('operationalImpactDefault inválido')
        err.status = 400
        throw err
    ...
    // VMRS (mapeo opcional recomendado)
    if (out.vmrs && typeof out.vmrs === 'object') {
        out.vmrs = {
            ...
            jobCode: cleanVmrsCode(out.vmrs.jobCode),
        }

        // Normalización por niveles
        if (!out.vmrs.systemCode) out.vmrs.componentCode = ''
        if (!out.vmrs.componentCode) out.vmrs.jobCode = ''

        // Consistencia: si componentCode existe, debería “colgar” del systemCode
        if (out.vmrs.systemCode && out.vmrs.componentCode && !out.vmrs.componentCode.startsWith(out.vmrs.systemCode)) {
            out.vmrs.componentCode = ''
            out.vmrs.jobCode = ''
        }
    } else {
        delete out.vmrs
...
export async function list(req, res, next) {
    try {
        ...
        // Filtros VMRS (opcionales)
        if (req.query.vmrsSystemCode) {
            const c = cleanVmrsCode(req.query.vmrsSystemCode)
            if (c) filter['vmrs.systemCode'] = c
        }
        if (req.query.vmrsComponentCode) {
            const c = cleanVmrsCode(req.query.vmrsComponentCode)
            if (c) filter['vmrs.componentCode'] = c
        }
        if (req.query.vmrsJobCode) {
            const c = cleanVmrsCode(req.query.vmrsJobCode)
            if (c) filter['vmrs.jobCode'] = c
        }
