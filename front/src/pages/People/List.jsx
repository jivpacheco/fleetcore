...
//   useEffect(() => {
//     load();
//   }, [page]); // recarga solo al cambiar página

//   const onSearch = async (e) => {
//     e.preventDefault();
//     setPage(1);
//     if (page === 1) await load(); // si ya estás en página 1, fuerza recarga
//     // await load()
//   };
...
//         setPage(page - 1); // retrocede una página si borraste el último de esta página
//       } else {
//         await load(); // recarga la misma página
//       }
//     } catch (err) {
//       console.error(err);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link className="px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-700" to="/people/new">
          Nueva persona
        </Link>
      </div>
      ...
      >
        <input
          className="border rounded px-3 py-2 md:col-span-2"
          placeholder="Buscar (RUN, nombre, email)..."
          value={q}
          onChange={(e) => setQ(e.target.value)}
        />
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="text-left p-2">Cargo</th>
              <th className="text-left p-2">Sucursal</th>
              <th className="text-left p-2">Conducción</th>
              <th className="text-left p-2">Activo</th>
              <th className="text-left p-2 w-56">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {items.map((it) => (
              <tr key={it._id} className="border-t">
                ...
                <td className="p-2">{it.branchId?.name || "—"}</td>
                <td className="p-2">
                  {it?.driverAuthorization?.isAuthorized ? (
