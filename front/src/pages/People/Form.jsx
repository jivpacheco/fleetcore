...
//   const cancelChanges = async () => {
//     if (!hasUnsavedChanges) {
//       navigate(`/people?q=${encodeURIComponent(saved?.dni || payload?.dni || '')}`);
//       return;
//     }
//     // Modo edición con cambios → "Cancelar" (restaurar)
//     if (isNew) {
...
export default function PeopleForm() {
  ...
  //   if (!hasUnsavedChanges) {
  //     navigate(
  //       `/people?q=${encodeURIComponent(saved?.dni || payload?.dni || "")}`,
  //     );
  //     return;
  //   }
  ...
  //     <UnsavedChangesGuard when={hasUnsavedChanges} />
  //             {dniDup && (
  //               <div className=\"md:col-span-3 text-sm text-red-600\">{dniDupMsg}</div>
  //             )}
  //             {checkingDni && (
  //               <div className=\"md:col-span-3 text-sm text-gray-500\">Validando RUN…</div>
  //             )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 border rounded p-4">
              ...
              <label className="text-sm">
                ...
                <select
                  ...
                  value={form.addressComuna}
                  onChange={(e) =>
                    setForm((s) => ({ ...s, addressComuna: e.target.value }))
                  }
                  disabled={!form.addressRegion}
                >
                  <option value="">— Selecciona comuna —</option>
