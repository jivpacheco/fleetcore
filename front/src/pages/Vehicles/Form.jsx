...
// // -----------------------------------------------------------------------------
// // Ficha de Vehículos (Básico, Técnico, Documentos, Medios, Inventario, Accidentes, Combustible)
// // - Estado: ahora al final del bloque Básico; cargado desde Catálogo (muestra etiqueta, guarda código).
// // - Año: min=1950, max=(AñoActual+1) + validación.
// // - Legal: Padrón con 3 fechas; nuevos bloques Revisión Técnica y Permiso de Circulación.
// // - Apoyos: iniciar/finalizar reemplazo (con auditoría).
// // - Auditoría: resumen compacto (últimos 5) + link a ver todo.
// // - Medios: carrusel al click, eliminación estable, auto-clasifica PDF.
// // -----------------------------------------------------------------------------
// import { useEffect, useMemo, useState } from 'react';
// import { api } from '../../services/http';
// import { useNavigate, useParams } from 'react-router-dom';
// import MediaUploader from '../../components/Vehicle/VehicleMediaUploader';
// import MediaGalleryModal from '../../components/Vehicle/MediaGalleryModal';
// import AuditSummary from '../../components/Vehicle/AuditSummary';
// import {
//   uploadVehiclePhoto, uploadVehicleDocument,
//   deleteVehiclePhoto, deleteVehicleDocument,
//   startSupport, finishSupport,
//   fetchVehicleAudit, fetchVehiclesByBranch
// } from '../../api/vehicles.api';
...
//       circulationPermit: { number:'', issuer:'', issuedAt:'', validTo:'' },
//     },
//     // Support (solo display; el payload real se maneja por endpoints dedicados)
//   });

//   function update(field, val) {
//     if (field === 'branch' || field === 'year' || field === 'status') {
//       setForm(f => ({ ...f, [field]: val }));
//     } else {
//       setForm(f => ({ ...f, [field]: (typeof val === 'string' ? U(val) : val) }));
//     }
...
//     // Auditoría (últimos 10 solo para panel completo)
//     const aud = await fetchVehicleAudit(id, { page: 1, limit: 10 });
...
//     if (!id) return;
//     setLoading(true);
//     loadVehicle().catch((err) => setError(err?.response?.data?.message || 'No se pudo cargar el vehículo'))
//       .finally(() => setLoading(false));
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [id]);
...
//   // --------- Apoyos ----------
//   useEffect(() => {
//     // al cambiar sucursal de apoyo, cargamos sus vehículos
//     if (!supportBranch) { setSupportVehicles([]); setSupportTarget(''); return; }
//     fetchVehiclesByBranch(supportBranch)
//       .then(({ data }) => {
//         const list = data?.items || data?.data?.items || data?.data || data?.list || [];
//         setSupportVehicles(list);
//       })
//       .catch(() => { setSupportVehicles([]); setSupportTarget(''); });
//   }, [supportBranch]);

//   const handleStartSupport = async () => {
//     if (!id) return alert('Guarda el vehículo antes');
//     if (!supportBranch || !supportTarget) return alert('Selecciona sucursal y vehículo a reemplazar');
//     await startSupport(id, { targetBranch: supportBranch, targetVehicle: supportTarget });
//     await refresh();
//     // limpiar selección
//     setSupportBranch(''); setSupportTarget('');
//   };
...
//     : ['ACTIVE','SUPPORT','IN_REPAIR','OUT_OF_SERVICE','RETIRED'].map(s => <option key={s} value={s}>{s}</option>);
...
//                   required
//                 />
//                 <p className="text-xs text-slate-500 mt-1">Permitido: 1950–{YEAR_MAX}</p>
//               </div>
...
//               <h3 className="font-medium text-slate-700">Servicios de Apoyo a otras Sucursales</h3>
//             </div>
//             <div className="p-4 grid sm:grid-cols-3 gap-4 items-end">
//               <div>
//                 <label className="block text-sm font-medium text-slate-600 mb-1">Sucursal destino</label>
//                 <select
//                   value={supportBranch}
...
//                 >
//                   <option value="">Selecciona vehículo</option>
//                   {supportVehicles.map(vh=>(
//                     <option key={vh._id} value={vh._id}>
//                       {vh.internalCode} — {vh.plate}
//                     </option>
//                   ))}
//                 </select>
//               </div>
...
//                 {!vehicle?.support?.active ? (
//                   <button type="button" onClick={handleStartSupport} className="px-3 py-2 bg-blue-600 text-white rounded w-full">
//                     Iniciar Reemplazo
//                   </button>
//                 ) : (
//                   <button type="button" onClick={handleFinishSupport} className="px-3 py-2 bg-emerald-600 text-white rounded w-full">
//                     Finalizar Reemplazo
//                   </button>
//                 )}
//               </div>

//               {/* Info de estado del reemplazo */}
//               {vehicle?.support?.active && (
//                 <div className="sm:col-span-3 mt-2">
//                   <div className="text-sm">
//                     <span className="font-medium">En reemplazo desde:</span>{' '}
//                     {vehicle.support.startedAt ? new Date(vehicle.support.startedAt).toLocaleString() : '—'}
//                   </div>
//                   <div className="mt-1 text-lg font-bold text-red-600">
//                     {vehicle.support.replacementCode}
//                   </div>
//                 </div>
//               )}
//             </div>
...
//           <div className="bg-white shadow rounded-xl border">
//             <div className="px-4 py-3 border-b bg-slate-50 rounded-t-xl">
//               <h3 className="font-medium text-slate-700">Auditoría (últimos eventos)</h3>
//             </div>
//             <div className="p-4">
//               <AuditSummary
//                 items={auditItems}
//                 onOpenFull={() => navigate(`/vehicles/${id}?tab=AUDITORIA`)}
//               />
//             </div>
//           </div>
...
//                   <div className="font-medium mb-1">Fotos / Videos</div>
//                   <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
//                     {(vehicle.photos||[]).map((ph, i)=>(
//                       <div key={ph._id} className="text-xs">
//                         {/^(mp4|mov|webm)$/i.test(ph.format || '') ? (
//                           <video className="w-full h-24 rounded border object-cover cursor-pointer" controls onClick={()=>openGalleryAt(i)}>
//                             <source src={ph.url} />
//                           </video>
//                         ) : (
//                           <img src={ph.url} alt={ph.title||''} className="w-full h-24 object-cover rounded border cursor-pointer"
...
//               items={(vehicle.photos||[]).map(p => ({ url: p.url, format: p.format, title: p.title }))}
...
// // //// actualizacion 22/10/2025 13:30 /// menos ajustado
// // front/src/pages/Vehicles/Form.jsx
// // -----------------------------------------------------------------------------
// // Ficha de Vehículos (Básico, Técnico, Documentos, Medios, ...)
// // - Estado: etiqueta desde catálogo (select muestra label, guarda code).
// // - Botón "Iniciar reemplazo": deshabilitado hasta sucursal + vehículo.
// // - Confirmación al abandonar con cambios sin guardar (beforeunload).
// // - Manejo de fechas a Date (evita CastError) y upper-case salvo fechas.
// // - Auditoría: resumen (usa tu componente existente si ya lo tienes).
// // - Mantiene tu UX estable; sólo agrega mejoras pedidas.
// // -----------------------------------------------------------------------------
...
// import MediaGalleryModal from '../../components/Vehicle/MediaGalleryModal';
// import AuditSummary from '../../components/Vehicle/AuditSummary';
// import TicketsBlock from '../../components/Vehicle/TicketsBlock';
...
//   function update(field, val) {
//     if (['branch','year','status'].includes(field)) {
//       setForm((f)=>({ ...f, [field]: val }));
//     } else {
//       setForm((f)=>({ ...f, [field]: (typeof val === 'string' ? U(val) : val) }));
//     }
...
//   useEffect(() => {
//     if (!id) {
//       initialFormRef.current = structuredClone(form);
//       return;
//     }
//     setLoading(true);
//     loadVehicle()
...
//   // --------- Warn al abandonar con cambios sin guardar ---------
//   useEffect(() => {
//     const hasChanges =
...
//     try {
//       const required = ['plate','internalCode','status','type','brand','model','year','color','branch'];
//       for (const k of required) {
//         if (!String(form[k] ?? '').trim()) throw new Error(`El campo ${k} es obligatorio`);
//       }
//       const yearNum = Number(form.year);
//       if (!Number.isFinite(yearNum) || yearNum < 1950 || yearNum > (new Date().getFullYear()+1)) {
//         throw new Error(`Año inválido. Debe estar entre 1950 y ${new Date().getFullYear()+1}.`);
//       }
...
//   // --------- Media ----------
//   const canUpload = useMemo(()=>Boolean(id), [id]);
//   const refresh = async ()=> { if (id) await loadVehicle(); };
//   const handleUploadPhoto = async ({ file, category='BASIC', title='' }) => {
//     if (!id) return alert('Guarda el vehículo antes de subir medios');
//     await uploadVehiclePhoto(id, { file, category, title });
//     await refresh();
//   };
//   const handleUploadDoc = async ({ file, category, label }) => {
//     if (!id) return alert('Guarda el vehículo antes de subir documentos');
//     await uploadVehicleDocument(id, { file, category, label });
//     await refresh();
//   };
//   const handleDeletePhoto = async (photoId) => {
...
//   const canStartSupport = Boolean(supportBranch && supportTarget);

//   const handleStartSupport = async () => {
//     if (!id) return alert('Guarda el vehículo antes');
//     if (!canStartSupport) return alert('Selecciona sucursal y vehículo a reemplazar');
//     await startSupport(id, { targetBranch: supportBranch, targetVehicle: supportTarget });
//     await refresh();
//     setSupportBranch(''); setSupportTarget('');
//   };
...
//         </div>
//         <nav className="flex gap-2">
//           {['BASICO','TECNICO','DOCUMENTOS','MEDIOS','TICKETS'].map(t=>(
//             <button type="button" key={t}
//               onClick={()=>setTab(t)}
//               className={`px-3 py-1.5 rounded ${tab===t?'bg-blue-600 text-white shadow-[0_0_0_3px_rgba(37,99,235,0.25)]':'bg-white border'}`}>
//               {t==='BASICO'?'Básico':t==='TECNICO'?'Técnico':t}
//             </button>
//           ))}
//         </nav>
//       </header>
...
//               {/* Estado (al final del bloque básico) */}
//               <div className="sm:col-span-2">
//                 <label className="block text-sm font-medium text-slate-600 mb-1">Estado</label>
//                 <select
//                   value={form.status}
...
//                     disabled={!canStartSupport}
//                     className={`px-3 py-2 rounded w-full ${canStartSupport ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500 cursor-not-allowed'}`}
//                   >
//                     Iniciar Reemplazo
//                   </button>
//                 ) : (
...
//                   </div>
//                   <div className="mt-1 text-lg font-bold text-red-600">
//                     {vehicle.support.replacementCode} {vehicle.support.originalInternalCode ? `(${vehicle.support.originalInternalCode})` : ''}
//                   </div>
//                 </div>
//               )}
//             </div>
...
//       {tab==='TECNICO' && (
//         <div className="space-y-4">
//           {/* ... tu contenido técnico existente ... */}
//           <div className="flex justify-end gap-3">
//             <button type="button" onClick={()=>navigate('/vehicles')} className="px-3 py-2 border rounded">Volver</button>
//             <button type="submit" disabled={saving} className="px-3 py-2 bg-blue-600 text-white rounded">
//               {saving ? 'Guardando…' : (id ? 'Guardar cambios' : 'Guardar')}
...
//       {tab==='MEDIOS' && (
//         <div className="space-y-4">
//           {/* ... tu uploader + listado de medios existente ... */}
//           <div className="flex justify-end">
//             <button type="button" onClick={()=>navigate('/vehicles')} className="px-3 py-2 border rounded">Volver</button>
//           </div>
//         </div>
