    ...
    <div>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Listado y estado de la flota</p>
        </div>
        <div className="flex items-center gap-2">
          <input className="input border rounded px-3 py-2" placeholder="Buscar por código/placa"
            defaultValue={q} onChange={(e)=> setQuery(KEY, e.target.value)} />
          <LimitSelect value={limit} onChange={(val)=> setLimit(KEY, val)} />
          <button className="btn btn-primary px-3 py-2 rounded text-white">Nuevo</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border">
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {isLoading && <tr><td colSpan={6} className="px-4 py-6 text-center">Cargando…</td></tr>}
              {error && <tr><td colSpan={6} className="px-4 py-6 text-center text-red-600">Error al cargar</td></tr>}
              {data?.docs?.map(v => (
                <tr key={v._id} className="border-t">
                  <td className="px-4 py-2">{v.code || '-'}</td>
                  <td className="px-4 py-2">{v.plate || '-'}</td>
                  <td className="px-4 py-2">{v.model || '-'}</td>
                  <td className="px-4 py-2">{v.branchName || v.branchId || '-'}</td>
                  <td className="px-4 py-2">{v.status || '-'}</td>
                  <td className="px-4 py-2 text-right">
                    <button className="text-[var(--fc-secondary)] mr-2">Ver</button>
              ...
              {!isLoading && !error && (data?.docs?.length ?? 0) === 0 &&
                <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-500">Sin resultados</td></tr>
              }
            </tbody>
          </table>
        </div>
