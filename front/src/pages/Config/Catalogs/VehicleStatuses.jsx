...
export default function VehicleStatusesCatalog() {
  ...
  const filtered = useMemo(() => {
    const qq = q.trim().toLowerCase()
    if (!qq) return items
    return items.filter(it => `${it.code || ''} ${it.label || ''}`.toLowerCase().includes(qq))
  }, [items, q])
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-bold">Catálogo: Estados de Vehículo</h1>
        <p className="text-sm text-gray-600">
          “Código” es la clave técnica (backend). “Nombre” es lo visible para el usuario.
        </p>
      </div>
      ...
      <form onSubmit={onSubmit} className="border rounded p-4 grid grid-cols-1 md:grid-cols-4 gap-3 bg-white">
        ...
        <div className="md:col-span-4 flex gap-2">
          <button type="submit" className="px-3 py-2 rounded bg-[var(--fc-primary)] text-white">
      ...
      <div className="border rounded overflow-hidden bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!filtered.length && (
              <tr><td className="p-3 text-gray-500" colSpan="4">{loading ? 'Cargando…' : 'Sin registros'}</td></tr>
            )}
          </tbody>
        </table>
      </div>
